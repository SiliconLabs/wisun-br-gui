...
#include "ws_br_agent_utils.h"
 ...
 /**
 * @brief Parse a single configuration line and update settings
 * @param line The configuration line to parse
 * @param settings Pointer to settings structure to update
 * @return WS_BR_AGENT_RET_OK on success, WS_BR_AGENT_RET_ERR on error
 */
static ws_br_agent_ret_t parse_config_line(const char *line, ws_br_agent_settings_t *settings)
{
  ...
  if (strcmp(key_start, "network_name") == 0) {
    strncpy(settings->network_name, value, WS_BR_AGENT_NETWORK_NAME_SIZE);
    settings->network_name[WS_BR_AGENT_NETWORK_NAME_SIZE] = '\0';
    ws_br_agent_debug("Configure network name: %s\n", settings->network_name);
    ...
    if (ws_br_agent_utils_str_to_val(value, ws_br_agent_nw_size_strs, 
      ...
      ws_br_agent_debug("Configure network size: %s (%u)\n", value, settings->network_size);
    } else {
    ...
    if (ws_br_agent_utils_str_to_val(value, ws_br_agent_keychain_strs, 
      ...
      ws_br_agent_debug("Configure keychain: %s (%u)\n", value, settings->keychain);
    } else {
    ...
    if (settings->phy.type == WS_BR_AGENT_PHY_CONFIG_FAN11 && 
      ...
      ws_br_agent_debug("Configure regulatory domain: %s (%u)\n", value, settings->phy.config.fan11.reg_domain);
    }
